...
class Cierremensual(View):
    @method_decorator(csrf_exempt)
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(request, *args, **kwargs)
    
    def get(self, request, *args, **kwargs):
        cierremensual = CierreMensualInsumo.objects.all()
        insumos = Insumos.objects.all()
        unidades = UnidadMedida.objects.all()
        categorias = Categoria.objects.all()
        cierrefecha = CierreMensual.objects.all()

        page = request.GET.get('page', 1)
        try:
            paginator = Paginator(cierremensual, 20)
            cierremensual = paginator.page(page)
        except:
            raise Http404
        
        ctx = {
            'entity': cierremensual,
            'insumos': insumos,
            'unidades': unidades,
        ...
        return render(request, 'frontweb/cierremensual.html', ctx)
...
class Maestroins(View):
    ...
    def post(self, request, *args, **kwargs):
        if request.POST['action'] == 'enviar_cierre':
            ...
            datos = []
            for i in insid:
                datos.append(i.split('_'))
            algo = ""
            count = 0
            for i in datos:
                fkinsumo = Insumos.objects.get(id_insumos = int(i[0]))
                cierre = CierreMensualInsumo.objects.filter(fk_id_insumos = fkinsumo).exists()
                if cierre == False:
                    count += 1
                    if count == 1:
                    ...
                    precio = i[2]
                    fkinsumo = Insumos.objects.get(id_insumos = int(i[0]))
                    cierremensualinsumo = CierreMensualInsumo.objects.create(id_cierre_mensual_insumo = idcierremensualinsumo, saldo= saldo, precio = precio, estado = 1, fk_id_cierre_mensual = fkcierremensual, fk_id_insumos = fkinsumo)
